...
# if no save path is provided, it is saved in the same location as the data
if SAVE_PATH is None: SAVE_PATH = DATA_PATH

# number of millions of molecules (this is our total database size)
t_mol = 1358.853531
...
for i in range(len(all_hyperparas)):
    with open(SAVE_PATH+'/iteration_'+str(n_it)+'/simple_job/simple_job_'+str(count)+'.sh', 'w') as ref:
        ...
        hyp_args = '-os {} -bs {} -n_estimators {} -max_depth {} -learn_rate {} -bin_array {} -min_child_weight {} -cf {}'.format(*all_hyperparas[i])
